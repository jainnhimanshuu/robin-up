...
export class CommonUtils {
  ...
  public static jsonSafeStringify(
    str: DeepReadonly<IUnknownRecord>
  ): string | null {
    try {
  ...
  public static stringifyError(error: unknown): string {
    ...
    Logger.logError("error string", errString);
  ...
  static getObjVal(
    ...
    return keys.reduce(
      (prev: DeepReadonly<IUnknownRecord>, curr: string) =>
        prev?.[curr] as IUnknownRecord,
      object
    );
  }
  ...
  static addLink(
    ...
    for (const attr in attribute) {
      s.setAttribute(
        attr,
        (attribute[attr] ? attribute[attr] : null) as string
      );
    }
    s.innerHTML = text ? text : "";
  ...
  static dispatchEvent(
    eventName: string,
    data?: DeepReadonly<IUnknownRecord>
  ): void {
  ...
  public static safeAtob = (hourlyPay: string): string => {
    try {
      ...
      Logger.logError("[Utils] Unable to parse atob", hourlyPay, err);
  ...
  public static isDeepEqual(
    ...
    return x && y && tx === "object" && tx === ty
      ? ok(x).length === ok(y).length &&
          ok(x).every((key) =>
            CommonUtils.isDeepEqual(
              x[key] as IUnknownRecord,
              y[key] as IUnknownRecord
            )
          )
      : x === y;
  }

  public static getUniqueElementsFromArray(
    rawArray: DeepReadonly<unknown[]>
  ): unknown[] {
  ...
  public static shuffleArray(
    array: DeepReadonly<React.ReactNode[]>
  ): React.ReactNode[] {
  ...
  public static getPageLoadTime(): number {
    ...
    if (window.performance) {
      try {
        ...
        Logger.logError(
          "[ERROR]: getEntriesByType not supported by browser",
          e
        );
      }
    }
    ...
    if (window.performance?.timeOrigin) {
      try {
        ...
        if (window.performance?.now()) {
          if (
            timeLoad !== 0 &&
            Math.abs(window.performance.now() - timeLoad) > 1000
          ) {
            Logger.logError(
              `time log err::${timeLoad}::${window.performance.now()}`
            );
          }
        }
      } catch (e) {
        Logger.logError("[ERROR]: calculating timeload through time origin", e);
      }
    }
  ...
  public static convertSnakeToCamel(
    obj: DeepReadonly<IUnknownRecord>
  ): IUnknownRecord {
  ...
  public static convertCamelToSnake(
    obj: DeepReadonly<IUnknownRecord>
  ): IUnknownRecord {
    if (typeof obj !== "object" || obj === null) {
      return obj; // Return non-object values as is
  ...
  public static getCookieFromString(
    ...
    const cookieArray = cookieString
      .split(";")
      .map((cookieStr) => decodeURIComponent(cookieStr));
    ...
    for (let cookieStr of cookieArray) {
      ...
      if (cookieStr.indexOf(nameWithEq) === 0) {
        const returnStr =
          cookieStr.substring(nameWithEq.length, cookieStr.length) || "";
